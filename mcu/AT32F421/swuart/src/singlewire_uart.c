...
typedef struct
{
    ...
    uint32_t dma_stream;   // DMA stream identifier for libopencm3
    uint32_t dma_full_flag;  // DMA full flag (for libopencm3 compatibility)
    uint32_t dma_half_flag;  // DMA half flag (for libopencm3 compatibility)
} sw_uart_config_t;
...
// libopencm3 DMA register structure for fast configuration
typedef struct
{
...
static inline void sw_uart_configure_dma_rx(void)
{
    uint32_t dma_base = sw_uart_config.dma;
    uint8_t channel = sw_uart_config.dma_channel;
    ...
    *(volatile dma_fast_config_t *)ccr_addr = dma_rx_config;
}
...
{
    // Calculate CCR address: DMA_BASE + CCR_OFFSET + (CHANNEL_SIZE * (channel - 1))
    uint32_t ccr_addr = sw_uart_config.dma + DMA_CCR_OFFSET + (DMA_CHANNEL_SIZE * (sw_uart_config.dma_channel - 1));
    
    dma_fast_config_t tx_config = dma_tx_config; // Copy template

    // Set runtime values
    tx_config.cndtr = bit_count;
    tx_config.cpar = GPIOB_BSRR_ADDR;  // Use libopencm3 style address
    tx_config.cmar = (uint32_t)sw_uart_tx_dma_buffer;

    // Disable channel first (clear EN bit)
    *(volatile uint32_t *)ccr_addr &= ~DMA_CCR_EN;

    // Fast block copy - struct is aligned to register layout
    *(volatile dma_fast_config_t *)ccr_addr = tx_config;
}
...
void sw_uart_init(sw_uart_config_t *config)
{
    ...
    rcc_periph_clock_enable(sw_uart_config.timer_clk); // Enable timer clock

    // Configure timer for bit-rate sampling using libopencm3
    timer_set_mode(sw_uart_config.timer, TIM_CR1_CKD_CK_INT, TIM_CR1_CMS_EDGE, TIM_CR1_DIR_UP);
    timer_enable_preload(sw_uart_config.timer);
    timer_continuous_mode(sw_uart_config.timer);

    nvic_set_priority(sw_uart_config.dma_irq, 4);
    nvic_enable_irq(sw_uart_config.dma_irq);

     libopencm3_atomic_exti_bit_modify(sw_uart_config.gpio_pin_num, 0); //disable the exti interrupt for now

    nvic_set_priority(sw_uart_config.exti_irq, 0); // high as we dont want to miss the start bit
    nvic_enable_irq(sw_uart_config.exti_irq);
    // Start in RX mode
    sw_uart_enable_rx();
...
void singlewire_uart_init()
{
    ...
    uart_config.dma_stream = SWUART_DMA_STREAM;          // DMA stream identifier
...
static inline void sw_uart_enable_rx(void)
{
    ...
    gpio_mode_setup(sw_uart_config.gpio_port, GPIO_MODE_INPUT, GPIO_PUPD_PULLDOWN, sw_uart_config.gpio_pin);
...
void exti4_15_isr (void)
{
    ...
    if (!gpio_get(sw_uart_config.gpio_port, sw_uart_config.gpio_pin))
    {
        ...
        exti_reset_request(sw_uart_config.exti_line);
        return;
    }
    ...
    timer_enable_counter(sw_uart_config.timer);
...
static inline void sw_uart_start_dma_sampling(void)
{
    ...
    dma_enable_transfer_complete_interrupt(sw_uart_config.dma, sw_uart_config.dma_channel);
    dma_enable_half_transfer_interrupt(sw_uart_config.dma, sw_uart_config.dma_channel);
    dma_enable_transfer_error_interrupt(sw_uart_config.dma, sw_uart_config.dma_channel);
    nvic_enable_irq(sw_uart_config.dma_irq);
    nvic_set_priority(sw_uart_config.dma_irq, 4);
    dma_enable_channel(sw_uart_config.dma, sw_uart_config.dma_channel);
...
inline void sw_uart_dma_complete_handler(void)
{
    ...
    if (dma_get_interrupt_flag(sw_uart_config.dma, sw_uart_config.dma_channel, DMA_TCIF) ||
        dma_get_interrupt_flag(sw_uart_config.dma, sw_uart_config.dma_channel, DMA_HTIF))
    {
        ...
        if (uart_state == SENDING)
        {
            ...
            dma_clear_interrupt_flags(sw_uart_config.dma, sw_uart_config.dma_channel, DMA_TCIF | DMA_HTIF);
            ...
            gpio_clear(sw_uart_config.gpio_port, sw_uart_config.gpio_pin);
        ...
        {
            bool is_full = dma_get_interrupt_flag(sw_uart_config.dma, sw_uart_config.dma_channel, DMA_TCIF);
...
inline static void sw_uart_setup_edge_detection(void)
{
    ...
    exti_select_source(sw_uart_config.exti_line, sw_uart_config.gpio_port);
    // Configure EXTI for rising edge detection using libopencm3
    exti_set_trigger(sw_uart_config.exti_line, EXTI_TRIGGER_RISING);
    ...
    exti_reset_request(sw_uart_config.exti_line); // Clear pending interrupt
    // Enable EXTI interrupt atomically using libopencm3 bit-banding
    libopencm3_atomic_exti_bit_modify(sw_uart_config.gpio_pin_num, 1);
...
inline static void sw_uart_start_tx_dma(void)
{
  ...
  // we need to have the 1.5 bit period immediately active
    ...
    timer_enable_preload(sw_uart_config.timer);
    timer_set_period(sw_uart_config.timer, TX_START_DELAY); // wait a while
    timer_generate_event(sw_uart_config.timer, TIM_EGR_UG);
    ...
    // Enable Timer DMA request on overflow using libopencm3
    timer_enable_irq(sw_uart_config.timer, TIM_DIER_UDE);
    dma_clear_interrupt_flags(sw_uart_config.dma, sw_uart_config.dma_channel, DMA_TCIF | DMA_HTIF | DMA_TEIF); // lets clear the flag, just to be very certain
    // Enable DMA channel and interrupts using libopencm3
    dma_enable_transfer_complete_interrupt(sw_uart_config.dma, sw_uart_config.dma_channel);
    dma_enable_transfer_error_interrupt(sw_uart_config.dma, sw_uart_config.dma_channel);
    nvic_enable_irq(sw_uart_config.dma_irq);
...
void sw_uart_enable_tx(void)
{
    ...
    gpio_mode_setup(sw_uart_config.gpio_port, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, sw_uart_config.gpio_pin);
    gpio_set_output_options(sw_uart_config.gpio_port, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, sw_uart_config.gpio_pin);
